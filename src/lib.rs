...
fn climdistpy(
    ...
    let out: Vec<f64> = pool
        .install(|| core::climdistrs(&array, &affine, &samples_x, &samples_y, nrows, ncols))
...
fn dissimpy(
    ...
    let out: Vec<f64> = pool
        .install(|| core::dissimrs(&array, &affine, is_geo, bandwidth, radius, nsample, ncols, seed))
        .map_err(|er| PyRuntimeError::new_err(format!("Climsim failed: {er}")))?;
...
fn climsim_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(dissimpy, m)?)?;
    m.add_function(wrap_pyfunction!(climdistpy, m)?)?;
    Ok(())
}
